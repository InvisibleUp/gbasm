...
// ----------------------------------------------------------------------------
function Parser(file, stream, debug) {
    this.file = file;
    this.stream = stream;
    this.debug = debug;
...
Parser.prototype = {
    ...
    parseName(s, name) {
        ...
        if (s.peak('EQU')) {
            ...
            this.file.addConstant(name.value, this.file.lastrs, name.index);
            this.file.lastrs += 1;
    ...
    parseDS(s) {
        ...
        const size = s.peak('NUMBER') ? s.expect('NUMBER') : s.expect('EXPRESSION');
        if (s.peak('EXPRESSION')) {
    ...
    parseDB(s) {
        ...
        if (s.peak('STRING')) {
            while((value = s.get('STRING'))) {
                ...
                const string = value.value.split('');
                values.push.apply(values, string.map((c, i) => {
    ...
    parseInstruction(s, mnemonic) {
        ...
        switch(mnemonic) {
            ...
            // Extended loads
            case 'ldxa':
                this.parseLdExtendedMeta(s);
                break;

            // Extended arithmetic
            case 'addw':
                this.parseAddWideMeta(s);
                break;
            ...
            case 'decx':
                ...
                // decx [someLabel]
                s.expect('[');
                const right = s.expect('NUMBER_16BIT');
                s.expect(']');

                this.instruction(16, [0xFA], right);
                if (mnemonic === 'incx') {
                    this.instruction(4, [0x3C]);

                } else {
                    this.instruction(4, [0x3D]);
                }
                this.instruction(16, [0xEA], right);
                break;
            ...
            // Debugging
            case 'msg':
                this.parseDebugMessageInstruction(s);
                break;
            ...
            case 'brk':
                if (this.debug) {
                    ...
                    this.file.warning(
                        mnemonic.index,
                        '[debug warning] Debug breakpoints are not compiled in without the -d flag.'.yellow
                    );
                }
                break;
    ...
    parseDebugMessageInstruction(s) {
        ...
        if (this.debug) {
            ...
            this.file.warning(
                arg.index,
                '[debug warning] Debug messages are not compiled in without the -d flag.'.yellow
            );
        }

    },

    parseLdInstruction(s) {
    ...
    parseAndXorInstruction(s, mnemonic) {
    ...
    parseAddSubInstruction(s, mnemonic) {
    ...
    parseAddSubInstructionOperand(s, mnemonic) {
    ...
    parseShiftInstruction(s, mnemonic) {
    ...
    parseLdExtendedMeta(s) {
        if (s.peak('[')) {
            ...
            if (s.peak('REGISTER_HL_INCREMENT')) {
                ...
                // ldxa   [hli],h
                // ldxa   [hli],l
                if (s.peak('REGISTER_8BIT_NON_ACC')) {
                    ...
                    }[s.get('REGISTER_8BIT_NON_ACC')]]);

                    // ld   [hli],a
                    this.instruction(8, [0x22]);

                // ldxa   [hli],$ff
                } else if (s.peak('NUMBER_8BIT')) {
                ...
                const left = s.expect('NUMBER_16BIT');
                s.expect(']');
                s.expect('COMMA');
            ...
            const left = s.get('REGISTER_8BIT_NON_ACC');
            s.expect('COMMA');
            s.expect('[');
    ...
    parseAddWideMeta(s) {
        const left = s.expect('REGISTER_DOUBLE');
        s.expect('COMMA');

        if (s.peak('ACCUMULATOR')) {
            s.expect('ACCUMULATOR');

        } else if (s.peak('REGISTER_8BIT_NON_ACC')) {
            // ld a,reg
            this.instruction(4, [{
                a: 0x7F,
                b: 0x78,
            ...
            // ld a,$ff
            this.instruction(8, [0x3E], s.get('NUMBER_8BIT'), true);
            // TODO optimize if value is <= 3 by using inc REGISTER_DOUBLE instead
        }
        ...
        // add     a,L
        this.instruction(4, [reg[L] + m['add']]);
        ...
        // adc     a,H
        this.instruction(4, [reg[H] + m['adc']]);

        // sub     L
        this.instruction(4, [reg[L] + m['sub']]);
    ...
    parseMulMeta(s) {
        const mul = s.expect('REGISTER_8BIT');
        s.expect('COMMA');
        ...
        switch (s.get('POWER_OF_TWO')) {
            // TODO optimize for higher values
            case 128:
                this.instruction(8, [0xCB, reg[mul] + 0x20]);
            case 64:
                this.instruction(8, [0xCB, reg[mul] + 0x20]);
            case 32:
                this.instruction(8, [0xCB, reg[mul] + 0x20]);
    ...
    parseDivMeta(s) {
        const div = s.expect('REGISTER_8BIT');
        s.expect('COMMA');
        ...
        switch (s.get('POWER_OF_TWO')) {
            // TODO optimize for higher values
            case 128:
                this.instruction(8, [0xCB, reg[div] + 0x38]);
            case 64:
                this.instruction(8, [0xCB, reg[div] + 0x38]);
            case 32:
                this.instruction(8, [0xCB, reg[div] + 0x38]);
    ...
    parseBitInstruction(s, mnemonic) {
        ...
        // set 0-7,a|b|c|d|e|h|l
            ...
            const right = s.get('REGISTER_8BIT');
            if (mnemonic === 'bit') {
                this.instruction(8, [0xCB, {
                    a: 0x47,
                    b: 0x40,
    ...
    instruction(cycles, code, arg, isByte, isSigned, isBit) {
    ...
    // Error Handling ---------------------------------------------------------
    error(msg, expected, index) {
        Errors.ParseError(this.file, msg, expected, index);
    }
