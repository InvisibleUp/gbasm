...
Lexer.prototype = {
    ...
    parseLocalLabel(buffer, index, ch) {

        let name = `.${  String.fromCharCode(ch)}`;
        const at = index;

        ch = buffer[index];

        while(isNamePart(ch, name)) {
    ...
    parseOffsetOrMacroArg(buffer, index, sign) {
    ...
    parseBinary(buffer, index, digit) {
    ...
    parseDecimal(buffer, index, digit, isNegative) {

        let number = String.fromCharCode(digit);
    ...
    parseMacroArgName(buffer, index, ch) {
    ...
    parseString(buffer, index, delimiter) {
    ...
    token(type, value, index) {
        ...
        // Combine macro tokens with their name
            ...
            const token = new Token(type, value, index, this.file);
...
function isNamePart(c, name) {
    if (name.toLowerCase() === 'hl' && (c === 43 || c === 45 )) {
        return true; 
    }
...
function isDirective(value) {
    ...
    return value === 'DB' || value === 'DW'
        ...
        || value === 'INCLUDE' || value === 'RSSET'
        || value === 'RSRESET' || value === 'RB'
        || value === 'RW' || value === 'RL';
}
